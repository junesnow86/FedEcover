...
for round in range(ROUNDS):
    ...
    pruned_global_cnn, _ = prune_cnn(global_cnn, 0.2, position=0)
    heterofl_aggregate(
        pruned_global_cnn, all_client_models, pruned_indices_list, subset_sizes
    )

    _, test_acc, class_acc = test(pruned_global_cnn, device, test_loader, criterion)
    print(
        f"Round {round + 1}, Pruned-global Aggregated Test Acc: {test_acc:.4f}\tClass Acc: {class_acc}"
    )
    round_results["Pruned-global Aggregated"] = test_acc
    round_results_class["Pruned-global Aggregated"] = class_acc

    _, test_acc, class_acc = test(global_cnn, device, test_loader, criterion)
    print(
        f"Round {round + 1}, Whole Aggregated Test Acc: {test_acc:.4f}\tClass Acc: {class_acc}"
    )
    round_results["Whole Aggregated"] = test_acc
    round_results_class["Whole Aggregated"] = class_acc
...
with open("results/heterofl_more_different_p_unbalanced_classes.csv", "w") as csvfile:
